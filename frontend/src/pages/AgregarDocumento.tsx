...
const AgregarDocumento: React.FC = () => {
  ...
  useEffect(() => {
    const obtenerConvocatorias = async () => {
      try {
        const res = await axios.get('http://localhost:8000/api/v1/convocatorias');
  ...
  useEffect(() => {
    const obtenerAreas = async () => {
      ...
      try {
        const res = await axios.get(`http://localhost:8000/api/v1/convocatorias/${convocatoriaSeleccionada}/areas`);
